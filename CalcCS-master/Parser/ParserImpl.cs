...
namespace CalcCS.Parser
{
    ...
    class TermParser
    {
        ...
        public static bool IsApplicable(string source, int pos)
        {
            return Term2Parser.IsApplicable(source, pos);
        }
        ...
        public static IExpression Parse(string source, ref int pos)
        {
            IExpression left = Term2Parser.Parse(source, ref pos);
            ParserImpl.SkipSpaces(source, ref pos);
            if (pos == source.Length) return left;
            ...
            if (source[pos] == '*' || source[pos] == '/' || source[pos] == '%')
            {
                ...
                switch (op)
                {
                    ...
                    case '%': return new ModuloOperation(left, right);
                }
    ...
    class Term2Parser
    {
        ...
        public static IExpression Parse(string source, ref int pos)
        {
            ...
            if (source[pos] == '^')
            {
                ...
                switch (op)
                {
                    case '^': return new PowerOperation(left, right);
                }
